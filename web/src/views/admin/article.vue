        ...
        <el-form :model="attrs.add_form" label-width="80px" :rules="rules" ref="form_dom" label-position="left"
            ...
            <el-form-item label="类型" prop="type">
                <el-select v-model="attrs.add_form.type" filterable default-first-option :reserve-keyword="false"
                    placeholder="类型">
                    <el-option v-for="value, key in article_type" :key="key" :label="value" :value="parseInt(key)" />
                </el-select>
            </el-form-item>
            ...
            <el-form-item label="" prop="content_md">
                <v-md-editor v-model="attrs.add_form.content_md" height="550px" :disabled-menus="[]"
...
const validate = () => {
    store().get_userinfo().then((res) => {
        attrs.add_form.content_html = xss.process(VueMarkdownEditor.vMdParser.themeConfig.markdownParser.render(attrs.add_form.content_md));
        attrs.add_form.user = res.id
