...
const click_json = (data) => {
    console.log('assssssssssss');
    attrs_.json_data = data;
    attrs_.json_view = true;
    console.log(attrs_.json_view);
}
</script>
<template>
        ...
        <el-table-column v-else-if="i.type == 'json' && i.show" :align="i.align" :label="i.label" :prop="i.prop"
            ...
            <template #default="scope">
                <div>
                    ...
                    <t-jsonview :data="scope.row[i.prop]"></t-jsonview>
                </div>
            </template>
        </el-table-column>
    </template>
    ...
    <el-dialog v-model="attrs_.json_view" width="50%" top="0" :modal="false" style="scrollbar-width: 0;">
        <t-jsonview :data="attrs_.json_data"></t-jsonview>
    </el-dialog>
</template>
