...
const filter_method = (node, key) => { //树状结构过滤
...
<template>
  <div class="head">
    ...
    <Expand style="width: 2em; height: 2em; margin-right: 8px; color: white" v-if="store().toggle_side" @click="
      store().toggle_side = !store().toggle_side;
    store().toggle_side
      ? (store().menu_width = '50px')
      : (store().menu_width = '200px');
    " />
    <Fold style="width: 2em; height: 2em; margin-right: 8px; color: white" v-else @click="
      store().toggle_side = !store().toggle_side;
    store().toggle_side
      ? (store().menu_width = '50px')
      : (store().menu_width = '200px');
