...
def special_filter(request: HttpRequest, queryset: QuerySet, filter_dict):
    ...
    for i in filter_dict_.keys():
        ...
        if 'or[' in i:  # 多条件或者
            ...
            del filter_dict[i]
        if '__not' in i:  # 不等于查询
            # filter_dict[i.replace('__not', '')] = ~Q(**{i.replace('__not', ''): filter_dict[i]})
            queryset = queryset.exclude(**{i.replace('__not', ''): filter_dict[i]})
            del filter_dict[i]
    if or_filter != None:
...
def list_common(self, request: HttpRequest, *args):
    ...
    if (page and limit):
        for k, v in temp_dict.items():
            ...
            if v == '{}':
                filter_dict[k] = {}
            if v == 'null' and '__not' in k:
                filter_dict[k] = None
            if v == 'null' and '__not' not in k:
                del filter_dict[k]
            if v == 'None':
                filter_dict[k] = None
            if v == 'NaN':
                filter_dict[k] = float('nan')
