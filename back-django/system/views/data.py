...
class Data(ViewSet):
    ...
    def upload(self, request: Request):  # TODO:文件上传-上传接口
        ...
        logger.error(f"文件上传失败，文件名：{form}")
        # logger.error(f"文件上传失败，文件名：{form.data['name']}")
    ...
    #     if form.is_valid():
    #         f = form.save()
    #         logger.info(f"文件上传成功，文件名：{f.name}")  # 上传文件以file为key值
    ...
    # 微信注册
    @action(['post'], url_path='signin_wechat', url_name='微信注册', detail=False, permission_classes=[], authentication_classes=[])
    def signin_wechat(self, request: Request):
        openid = request.data.get("openid")
        if user.objects.filter(openid=openid).exists():
            return Response({"detail": "用户已存在"}, status=200)
        else:
            u = user.objects.create_user(openid=openid, username='admin',
                                         password='admin',
                                         is_super=False)
    # 生成验证码
    ...
    @ratelimit(key='ip', rate='1/s')
    @action(['get'], url_path='phone_captcha', url_name='生成手机验证码', detail=False, permission_classes=[], authentication_classes=[])
    def phone_captcha(self, request: Request):
        phone = request.GET.get('phone')
        s, v = gvcode.generate()  # 获取手机验证码，当前未实现
        key = hashlib.md5(('captcha-' + phone).encode()).hexdigest()
        cache.set(key, v, 60)
        output_buffer = BytesIO()
        s.save(output_buffer, format='JPEG')
        byte_data = output_buffer.getvalue()
    ...
    def querythreads(queries: dict, max_workers=10) -> dict:
        """
        多线程sql查询
        
        queries = {
            'key1': 'select * from table1',
            'key2': 'select * from table2'
        }
        """
        result = {}
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_query = {executor.submit(execute_query, query): key for key, query in queries.items()}
